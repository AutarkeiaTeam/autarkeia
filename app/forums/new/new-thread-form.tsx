...
function NewThreadFormInner({ authed }: { authed: boolean }) {
  const { t } = useI18n()
    ...
    return (
      <main className="min-h-screen bg-[#f5f7fa]">
        <div className="mx-auto max-w-2xl px-4 py-20 lg:px-8">
          ...
          <p className="mt-3 text-sm text-[#3d5166]">{t("forums.new.sign_in_copy")}</p>
          <Link
            href="/login"
...
export function NewThreadForm({ authed }: { authed: boolean }) {
  return (
    <Suspense fallback={<NewThreadFallback />}>
      <NewThreadFormInner authed={authed} />
    </Suspense>
  )
}
