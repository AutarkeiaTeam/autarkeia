...
type LibraryAccessTier = "free" | "pro"

export default function Library() {
  const { t } = useI18n()
  const [type, setType] = useState("All")
  const [subject, setSubject] = useState("All")
  const [accessTier, setAccessTier] = useState<LibraryAccessTier>("free")

  useEffect(() => {
    const user = supabaseClient.auth.user()
    const tier = user?.user_metadata?.tier
    setAccessTier(user && tier === "pro" ? "pro" : "free")
  }, [])
  ...
  const filtered = useMemo(() => {
    ...
    return accessTier === "pro" ? matchingItems : matchingItems.filter((item) => !item.isPro)
  }, [accessTier, type, subject])
